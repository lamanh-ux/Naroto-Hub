--[[
                            VO LAM ANH PROJECT
            This was made by Volamanh Naroto Hub ( discord.gg/quantumonyx )
            KEYSYSTEM UI built using claude ai
            Service by Luarmor.net
            Compiled by: Flazhy
            Copyright © 2022-2026 VOLAM ANH Team - All Rights Reserved.
]]--
local API_CONFIG = {
...
local FOLDER = "Volamanh Naroto Hub"
...
local function Notify(title, desc, accent, duration)
    pcall(function()
        StarterGui:SetCore("SendNotification", {
            Title = title or "Volamanh Naroto Hub",
            Text = desc or "",
...
local function LoadScript(tier, scriptPayload)
    if tier == "Free" then
        ...
        if url then
            ...
            warn("[Volamanh Naroto Hub] No free script found for GameId: " .. tostring(GameId))
        end
        ...
        if scriptPayload and type(scriptPayload) == "string" and #scriptPayload > 100 then
            task.spawn(function()
                ...
                if func then
                    ...
                    if not success then
                        warn("[Volamanh Naroto Hub] Runtime error: " .. tostring(runtimeErr))
                        Notify("Script Runtime Error", tostring(runtimeErr):sub(1, 80), Color3.fromRGB(255, 90, 110), 10)
                    end
                else
                    warn("[Volamanh Naroto Hub] Compilation error: " .. tostring(compileErr))
                    Notify("Script Compile Error", tostring(compileErr):sub(1, 80), Color3.fromRGB(255, 90, 110), 10)
                end
            end)
        else
            warn("[Volamanh Naroto Hub] Payload empty from server, loading Luarmor loader fallback...")
            LoadStockLoader()
        end
    end
end
...
local function ShowKeyUI()
    ...
    New("TextLabel", {
        ...
        Text = "Volamanh Naroto Hub — Key System",
        ...
        ResolveAndLoadKey(keyStr, {
            ...
            onFail = function(code, message)
                ...
                elseif code == "sdk_unreachable" then
                    ...
                    Notify("Volamanh Naroto Hub", "Could not reach Luarmor SDK. Try again.", Color3.fromRGB(255, 90, 110))
                    return
